...
const MONGODB_URI = "mongodb+srv://Baptiste13170:22MP4x0jUzQ96eX3@cluster0.3mq07dd.mongodb.net/?retryWrites=true&w=majority";
...
async function findLessThan(price) {
  try {
      const client = await MongoClient.connect(MONGODB_URI, { useNewUrlParser: true });
      const db = client.db(MONGODB_DB_NAME);
      console.log(`Connected to database: ${MONGODB_DB_NAME}`);
...
async function sortByDate() {
  try {
      ...
      const sortedProducts = products.sort((a, b) => a.date - b.date);
